...
assert(version.includes('adventure-companion-m4-03-build-9'),"central cache identity is stale");
...
assert(worker.includes('./service-worker-update.js'),"service-worker update controller is not cached");
assert(html.indexOf('src="service-worker-update.js"')<html.indexOf('src="app.js"'),"service-worker update controller must load before app.js");
for(const file of ['version.js','app.js','index.html','reliability.js','service-worker.js','service-worker-update.js']) assert(fs.statSync(file).size>0,`${file} is empty`);
